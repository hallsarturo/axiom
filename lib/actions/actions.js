...
// BOOKMARKS
export async function getBookmarksByPostId(postId) {
    try {
        let headers = { 'Content-Type': 'application/json' };
        const res = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL}/api/posts/bookmarks/${postId}`,
            {
                method: 'GET',
                headers,
            }
        ...
        const data = await res.json();
        console.log('getBookmarksByPostId data', data);
...
export async function putBookmarkByPostId(token, postId, useId) {
    try {
        let headers = { 'Content-Type': 'application/json' };
        // Only add Authorization header in dev, and only if running on client
        if (process.env.NODE_ENV === 'development') {
        ...
        console.log('putBookmarkByPostId response: ', res);
        if (!res.ok) {
            const { message } = await res.json();
            return { success: false, error: message };
        }
...
// COMMENTS
export async function getCommentsByPostId(postId, page = 1, pageSize = 20) {
    try {
        const url = new URL(
            `${process.env.NEXT_PUBLIC_API_URL}/api/comments/${postId}`
        );
        ...
        if (!res.ok) {
            return {
                success: false,
                error: 'Failed to fetch CommentsByPostID',
            };
        }

        const data = await res.json();
        // Add debug logging
        // console.log(`\n\ngetCommentsByPostID response for page ${page}:`, {
        //     postId: postId,
        //     imageSrc: data.comments?.userProfilePic,
        //     count: data.paperPosts?.length,
        ...
        console.error('Error fetching CommentsByPostID:', error);
        return { success: false, error: 'Error fetching CommentsByPostID' };
    }
}
