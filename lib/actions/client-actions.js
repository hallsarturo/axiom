...
export async function loginUser(values) {
    try {
        ...
        // Store token in localStorage for development environment
...
export async function logoutUser() {
    try {
        ...
        document.cookie =
            'token=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/; domain=.axiomlab.space; secure; samesite=none';

        // Client-side redirect instead of server revalidation
...
export async function sendOtpCode(code, token) {
    ...
    try {
        ...
        // Client-side navigation instead of revalidation
...
export async function getBookmarksByUserId(
    ...
    try {
        ...
        const url = new URL(
            `${process.env.NEXT_PUBLIC_API_URL}/api/posts/bookmarks-personal/${userId}`
        );
        url.searchParams.append('page', page);
        url.searchParams.append('pageSize', pageSize);
        const res = await fetch(url, {
        ...
        const data = await res.json();
        //console.log('getBookmarksByUserId data', data);
...
export async function getFollowingsById(userId) {
    try {
        let headers = { 'Content-Type': 'application/json' };
        const res = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL}/api/user/following/${userId}`,
            {
                method: 'GET',
                headers,
            }
...
export async function publishComment(token, postId, values) {
    try {
        ...
        console.error('publishComment error: ', error);
        return { success: false, error: 'Network error' };
    }
}
