...
import { putPostReaction, putCommentReaction } from '@/lib/actions/actions';
...
export const useReactionsStore = create((set, get) => ({
    ...
    handleCommentReaction: async (commentId, type, userId, token) => {
        ...
        try {
            ...
            // Fetch the latest data from server
            const updatedData = await fetchPost(commentId, token, userId);

            // Update store with actual server data
            if (updatedData) {
                get().setReactionData(commentId, updatedData, 'comment');
            }
        } catch (error) {
            console.error('Error handling reaction:', error);
