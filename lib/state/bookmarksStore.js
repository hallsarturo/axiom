...
export const useBookmarksStore = create((set, get) => ({
    ...
    handleBookmark: async (postId, userId, token) => {
        ...
        const newIsBookmarked = !currentState.isBookmarked;
        ...
        try {
            ...
            if (res.success) {
                // Important: Always fetch the real count after a successful toggle
                // This ensures the UI shows the accurate count from the server
                setTimeout(async () => {
                    await get().fetchBookmarkCount(postId);
                }, 300); // Small delay to allow server to process
            } else {
